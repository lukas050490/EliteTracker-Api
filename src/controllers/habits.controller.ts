...
export class HabitsController {
    ...
    store = async (request: Request, response: Response): Promise<Response> => {
        ...
        if (!habit.success) {
            const errors = ErrorMessageValidation(habit.error.issues)
    ...
    toggle = async (request: Request, response: Response) => {
        ...
        const validator = schema.safeParse(request.params)

        if (!validator.success) {
            const errors = ErrorMessageValidation(validator.error.issues)

            return response.status(422).json({ message: errors })
        }

        const findHabit = await habitModel.findOne({
            _id: validator.data.id,
            userId: request.user.id,
        })
        ...
        const isHabitCompletedOnDate = findHabit
            .toObject()
            ?.completedDates
            .find((item) => dayjs(String(item)).toISOString() === now)
